...
var pathType = true, //是否采用自动获取绝对路径。 false：将采用相对路径(需在下述变量中配置)
pathUrl = 'http://xu.sentsin.com/lily/lib/layer/', //相对路径url
...
layerRun = function(){
	var $ = jQuery, ready = {
		getPath : function(){
			...
			$('head').prepend('\n' + '<link rel="stylesheet" id="layerCss" type="text/css" href="'+ pathUrl +'skin/layer.css"  />' + ie6PNG);
		}, global : {
			iE6 : !-[1,] && !window.XMLHttpRequest,
			times : 0, //追寻索引
			config : {}
		}
	}, w = $(window), FUNC = Function();
	ready.append();
	...
	CLASS.prototype = {
		type : function(){
			...
			this.adjust();
			var et = $.now();
			this.layerE.attr('msec', (et-st) + 'ms');
		}, space : function(){ //层容器
			var times = ready.global.times, newFig = ready.global.config[times] = this.config;
			newFig.dialog = this.dialog;
			newFig.page = this.page;
			newFig.iframe = this.iframe;
			newFig.loading = this.loading;
			newFig.tips = this.tips;
			...
			frame = [
				...
				'<iframe id="xubox_iframe" name="xubox_iframe" onload="javascript:$(\'.xubox_layer\').find(\'.xubox_iframe\').removeClass(\'xubox_load\');" class="xubox_iframe" frameborder="0" src="' + this.iframe.src + '"></iframe>',				
		...
		}, pagetype : function(times){ //调整layer			
			...
			this.move();
		}, autoArea : function(times){ //自适应宽高
			var othis = this, layerE = $('#xubox_layer' + times), config = ready.global.config[times], page = config.page,
					...
					$(document).mousemove(function(M){
						if(ismove){
							...
							if(layerE.css('position') == 'fixed'){
								var y = M.pageY - moveY - w.scrollTop();	
							}else{
								var y = M.pageY - moveY;	
							}
			...
			this.layerS.off('click').on('click', function(){
				var index = $(this).attr('times');
				ready.global.config[index].shadeClose && LAYER.close(index);
			});
		}, IE6 : function(){
		...
		}, openLayer : function(othis){ //layer开启后全局类
			this.autoArea = function(index){ //自适应宽高
				if(typeof index != 'number'){
					return othis.autoArea(ready.global.times);
				}else{
					return othis.autoArea(index);
				}
			};
			this.getIndex = function(selector){ //获取layer当前索引
				return $(selector).parents('.xubox_layer').attr('times');	
			};
			this.getChildFrame = function(selector){ //获取子iframe的DOM
				return $("#xubox_iframe").contents().find(selector);	
			};
			this.getFrameIndex = function(){ //得到iframe层的索引，子iframe时使用
				return $('#xubox_iframe').parents('.xubox_layer').attr('times');
			};
			this.close = function(index){ //关闭layer
